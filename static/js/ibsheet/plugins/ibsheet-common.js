...
(function(window, document) {
  ...
  function freezePanes(obj, opt) {
    ...
    if (obj.options.Cfg.MultiRecord) {
      obj.showMessageTime({
        message: "멀티레코드 기능 사용 시 열고정은 사용할 수 없습니다.",
        func: function () {
          // 행고정
  ...
  _IBSheet.CommonOptions = {
    Cfg: {
        ...
        CanEdit: 0, // Edit 기능 Disabled
        Export: {
          Url: "/ibsheet8/jsp/",
        }, // 엑셀다운 URL
        FitWidth: true,
        InfoRowConfig: { // 시트 하단 건 수 표시
          Visible: 0, // 표시하지 않음 (Default)
          Layout: ["Count"],
          Space: "Bottom",
        }, // 건수 정보 표시
        GroupFormat:
    ...
    Def: {
      ...
      Row: {
        Menu: {
          //마우스 우측버특 클릭시 보여지는 메뉴 설정 (메뉴얼에서 Appedix/Menu 참고)
          Items: [{ Name: "틀 고정" }, { Name: "틀 고정 취소" }],
          OnSave: function (item, data) {
            //메뉴 선택시 발생 이벤트
            switch (item.Name) {
              case "틀 고정":
              ...
              colObj["OnChange"] = function(e){
                ...
                if(e.sheet.Cols[e.col]["EditEnum"]) {
                    ...
                    if(e.sheet.Cols[e.col].EditEnum.indexOf(v) <= 0) {
                      ...
                      //선택 값이 EditEnum의 몇번째 값인지 찾고 해당하는 EnumKeys를 대신 넣는다.
  ...
  window.IB_Preset = {
    ...
    // Status Type
    "STATUS"		: {Type: "Text"	, Align: "Center"	, Width: 50		, Formula: "Row.Deleted ? 'D' : Row.Added ? 'I' : Row.Changed ? 'U' : 'R'", Format: { 'I': '입력', 'U': '수정', 'D': '삭제', 'R': '' }},
    // DelCheck Type
